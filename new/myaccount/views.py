...
from myaccount.forms import FormRegistration, FormAuthentification, Account, UserUpdateForm
from django.conf import settings
from django.shortcuts import render, redirect
from django.contrib.auth.decorators import login_required
from myaccount.forms import UserUpdateForm

from myaccount.forms import FormRegistration, FormAuthentification
...
def register_view(request, *args, **kwargs):
    ...
    if request.POST:
        ...
        if form.is_valid():
            ...
            myaccount = authenticate(email=email, password=raw_password)
            login(request, myaccount)
...
def profil_view(request, *args, **kwargs):
    ...
    if account :
        ...
        return render(request, "myaccount/profil.html", context)
...
def update_user_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('myaccount:profile', user_id=user.id)
    else:
        form = UserUpdateForm(instance=user)
    
    context = {'form': form}
    return render(request, 'myaccount/update_account.html', context)